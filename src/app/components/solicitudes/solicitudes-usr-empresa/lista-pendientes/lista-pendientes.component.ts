...
export class ListaPendientesComponent implements OnInit, OnDestroy {
  ...
  obtenerSolicitudes(){
    ...
    this.srvSolicitud.obtenerSolicitudesPendientesXEmpresa(this.usuario.idempresa).subscribe({
      next:(solicitudes) => {
  ...
  buscar(){
    if (this.formularioBusqueda.invalid) {
      ...
      this.suscripcion.add(
        this.srvSolicitud.obtenerUsuariosXSolicitud(this.formularioBusqueda.value.busqueda, this.usuario.idempresa).subscribe({
          next:(solicitudes) =>{
  ...
  aprobar(sol: SolicitudUsuario){
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.srvSolicitud.aprobarSolicitudPendientesUsuarioEmpresa(sol.idautorizacion).subscribe({
          next:(solicitudAprobada) => {
            
            this.srvUsuario.updateUsuarioEmpresa(usr).subscribe({
              next:(u) => {
  ...
  rechazar(idsolicitud: number){
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.srvSolicitud.rechazarSolicitudPendientesUsuarioEmpresa(idsolicitud).subscribe({
