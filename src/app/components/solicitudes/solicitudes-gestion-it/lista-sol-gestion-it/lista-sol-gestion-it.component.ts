...
import { Pedido, PedidoXDetalle } from 'src/app/interfaces/pedido';
...
export class ListaSolGestionItComponent implements OnInit, OnDestroy {
  ...
  obtenerSolicitudes(){
    ...
    this.srvSolicitud.obtenerSolicitudes(this.usuario.idempresa, 0).subscribe({
      next:(solicitudes) => {
                ...
                next:(producto) => {
                  ...
                  this.srvEmpresa.obtenerEmpresa(solicitudes[i].idproveedor).subscribe({
                    next:(proveedor) =>{
                      ...
                      this.srvEstado.obtenerEstado(solicitudes[i].idestado).subscribe({
                        next:(estado) => {
